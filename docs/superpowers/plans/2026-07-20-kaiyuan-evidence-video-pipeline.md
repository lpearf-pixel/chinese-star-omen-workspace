...
- [ ] **Step 5: Commit with `git commit -m "feat(video): add evidence video package contracts"`.**
...
- [ ] **Step 1: Write failing tests using an injected fake timescale/ephemeris.** Cover UTC normalization, right ascension/declination/ecliptic coordinates, observer altitude/azimuth, angular separation, finite-value rejection, unknown bodies, and deterministic provenance hashes.
- [ ] **Step 2: Run the focused module and require RED from missing provider.**
- [ ] **Step 3: Implement dependency-injected Skyfield calculation.** Runtime loads only the configured local ephemeris file; errors are classified as `missing_ephemeris`, `unknown_body`, `invalid_time`, or `calculation_error`, never empty success.
- [ ] **Step 4: Rerun focused tests; expect pass without internet access.**
- [ ] **Step 5: Run `cd apps/star-omen && pytest -q tests/test_config_settings.py tests/video_pipeline/test_astronomy_provider_v1.py`.**
- [ ] **Step 6: Commit with `git commit -m "feat(video): add deterministic astronomy provider"`.**
...
- [ ] **Step 1: Add failing tests for exact object mapping, nearest-star angular mapping, ambiguous equal-distance candidates, low-confidence mapping, and unresolved aliases.**
- [ ] **Step 2: Add failing event tests for conjunction/near-passage, lunar phase marker, visibility-required failure, missing angular separation, and `insufficient_data`.**
- [ ] **Step 3: Implement the minimum matcher and detector with deterministic tie-breaking and explicit confidence/method metadata.**
- [ ] **Step 4: Run `cd apps/star-omen && pytest -q tests/video_pipeline/test_asterism_event_v1.py`; expect pass.**
- [ ] **Step 5: Commit with `git commit -m "feat(video): map astronomy events to Chinese asterisms"`.**
...
- [ ] **Step 1: Write failing tests with fakes proving official structured retrieval runs before official primary retrieval and filesystem fallback runs only when official primary is empty.**
- [ ] **Step 2: Add tests proving pending overlays never become `classical_quote`, generic transport/contract errors are not converted into no-hit, and citable references require source/locator/page/paragraph/heading/anchor/hash.**
- [ ] **Step 3: Add rule tests proving `insufficient_data`, `candidate_only`, conflict suppression, and primary-evidence preference remain visible in the video evidence bundle.**
- [ ] **Step 4: Implement the evidence builder by composing existing modules without duplicating retrieval or citation logic.**
- [ ] **Step 5: Run `cd apps/star-omen && pytest -q tests/video_pipeline/test_evidence_builder_v1.py tests/test_cli_evidence_audit_v2.py tests/test_rule_matcher.py`.**
- [ ] **Step 6: Commit with `git commit -m "feat(video): assemble citable astronomy evidence"`.**
...
- [ ] **Step 1: Write failing tests for the fixed 60–90 second structure, deterministic segment ordering, exact claim classes, and required source disclosure.**
- [ ] **Step 2: Add rejection tests for unsourced classical quotation, candidate-only quotation, unclassified text, deterministic/fatalistic promises, and wording that attributes modern interpretation to the ancient source.**
- [ ] **Step 3: Implement a template-only compiler; do not add a free-form LLM dependency in B9.**
- [ ] **Step 4: Verify the phrase `开口破局` appears only in a `modern_interpretation` segment and disclosure identifies it as contemporary cultural translation.**
- [ ] **Step 5: Run the focused tests; expect pass.**
- [ ] **Step 6: Commit with `git commit -m "feat(video): compile claim-labelled short video scripts"`.**
...
- [ ] **Step 1: Write snapshot tests for date, location, sky culture, projection, atmosphere/landscape state, object selection, camera movement, field of view, pauses, screenshot names, and safe relative output paths.**
- [ ] **Step 2: Add failure tests for absolute output paths, path traversal, unsupported object IDs, negative shot duration, and scene/event time mismatch.**
- [ ] **Step 3: Implement deterministic `.ssc` generation using only allowlisted commands and caller-selected relative screenshot names.**
- [ ] **Step 4: Add a parser-level smoke assertion that every generated screenshot is represented in the render manifest.**
- [ ] **Step 5: Run `cd apps/star-omen && pytest -q tests/video_pipeline/test_stellarium_script_v1.py`; expect pass.**
- [ ] **Step 6: Commit with `git commit -m "feat(video): generate Stellarium scene scripts"`.**
...
- [ ] **Step 1: Write failing tests for SRT numbering, millisecond formatting, no overlap, exact total duration, frame hash inventory, 1080x1920 output, and shell-safe argv construction.**
- [ ] **Step 2: Add rejection tests for missing frames, hash mismatch, audio shorter than the narration timeline, output outside package root, and `final` mode without audio.**
- [ ] **Step 3: Implement pure manifest and command construction; isolate `subprocess.run` behind `run_ffmpeg(...)` and do not execute it in unit tests.**
- [ ] **Step 4: Add `.gitignore` entries for `video_packages/`, frame sequences, WAV/M4A files, and MP4 files while preserving committed JSON/YAML fixtures.**
- [ ] **Step 5: Run focused tests; expect pass.**
- [ ] **Step 6: Commit with `git commit -m "feat(video): add subtitles and local media rendering"`.**
...
- [ ] **Step 1: Write RED tests proving the builder writes no partial directory when validation or serialization fails and refuses to overwrite an existing package.**
- [ ] **Step 2: Write gate tests requiring separate astronomy, classical evidence, editorial, and render approvals; reviewer identity and UTC timestamp are mandatory.**
- [ ] **Step 3: Add tests proving `partial_metadata_only`, `candidate_only`, ambiguous asterism, missing audio, or changed asset hashes block `publishable`.**
- [ ] **Step 4: Implement atomic staging-directory write followed by same-filesystem rename; use strict canonical JSON and SHA-256 inventory.**
- [ ] **Step 5: Run focused tests; expect pass.**
- [ ] **Step 6: Commit with `git commit -m "feat(video): add atomic packages and review gate"`.**
...
- [ ] **Step 1: Write CLI tests for successful dry package generation, strict JSON output, explicit non-zero exits, and absence of external process execution without `--execute`.**
- [ ] **Step 2: Create the July 21 input as a candidate with configurable observer coordinates and explicit `source_video_status="partial_metadata_only"`; do not hardcode an unverified Spica/角宿一 conclusion as verified.**
- [ ] **Step 3: Create the modern interpretation fixture with `phrase="开口破局"`, `claim_class="modern_interpretation"`, and `classical_quote=false`.**
- [ ] **Step 4: Implement Typer and fallback argparse commands using shared implementation functions.**
- [ ] **Step 5: Run `make video-pipeline-test`; then run `make video-plan-2026-07-21` against fake/offline fixtures and inspect every generated file.**
- [ ] **Step 6: Commit with `git commit -m "feat(video): expose evidence video pipeline CLI"`.**
...
- [ ] **Step 1: Add an end-to-end RED test that builds the July 21 candidate package and asserts exact claim counts, citable-reference requirements, disclosure text, shot inventory, `.ssc`, SRT, and blocked publish status before human/audio approval.**
- [ ] **Step 2: Add failure injection for tampered evidence hash, missing angular separation, candidate-only quotation, path traversal, and changed frame hash; each must fail before publishable output.**
- [ ] **Step 3: Register a named `Evidence-backed video pipeline gate` in `kaiyuan-stable-core.yml`.**
- [ ] **Step 4: Document local prerequisites and exact commands in the B9 runbook, including Stellarium user permissions for external screenshot directories and FFmpeg availability checks.**
- [ ] **Step 5: Record B9 tasks in `TASKS.md`, architecture decisions in `DECISIONS.md`, and RED/GREEN evidence in `WORK_LOG.md`; move status to `VERIFYING`.**
- [ ] **Step 6: Run focused tests, `make contracts-test`, `make text-core-test`, `make downstream-test`, `make upstream-test`, governance checks, and the new E2E gate.**
- [ ] **Step 7: Perform one manual local rendering smoke with Stellarium and FFmpeg into an ignored package directory; record versions, commands, output hashes, and remaining visual defects without committing media.**
- [ ] **Step 8: Open a draft PR targeting only `stable/kaiyuan-v2`; keep it draft until exact-head workflows pass and independent review has no unresolved Critical or Important findings.**
- [ ] **Step 9: After merge, record final head, workflow run IDs, squash merge SHA, and change B9 status to `DONE` in a docs-only closeout PR.**
