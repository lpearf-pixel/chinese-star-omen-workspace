...
**Goal:** Add a deterministic CI test that composes read-only observation capture, release artifact assembly, sealed bundle creation, and offline verification without accessing `local_kb_default`.
...
- Never access, create, write, delete, or recreate `local_kb_default`.
...
- [ ] **Step 1: Write a failing test that imports a not-yet-created `_run_release_evidence_gate` helper and asserts exact verified output.**
- [ ] **Step 2: Run `cd apps/local-kb-unified && PYTHONPATH=. pytest -q tests/test_release_evidence_e2e_v1.py`; expect RED from the missing helper.**
- [ ] **Step 3: Implement the minimum test-local deterministic health, meta, retrieval, and inspection adapters, capture three phases, assemble, bundle, and verify.**
- [ ] **Step 4: Rerun the focused test; expect `1 passed`.**
...
- Produces: protected-access audit and tamper regression tests.

- [ ] **Step 1: Add tests that fail until inspection rejects `local_kb_default` and after-switch manifest tampering raises `ReleaseArtifactError(drill_validation_failed)`.**
- [ ] **Step 2: Run the focused module and record the expected RED failures.**
- [ ] **Step 3: Complete the minimum test fixture audit and preserve the assembly exception; do not catch it as an empty result.**
- [ ] **Step 4: Rerun the focused module; expect all tests pass.**
...
- [ ] **Step 1: Add workflow step `Hermetic release evidence end-to-end gate` running the focused module after archive verification.**
- [ ] **Step 2: Run all six release-observation/drill/artifact/bundle/archive/e2e modules together and require all pass.**
- [ ] **Step 3: Move B8-T02 to VERIFYING; run contracts, text-core, downstream, upstream, release-drill, and governance gates.**
