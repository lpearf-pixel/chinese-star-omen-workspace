...
Current task: B9-G6-E1 closeout
B9 overall: VERIFYING
B9-G6: READY after closeout merge
...
- **Status:** `DONE` after this closeout merges
...
- **Status:** `READY` after closeout merge
...
```text
merge B9-G6-E1 docs-only closeout
→ run macOS G6 collector
→ upload and verify evidence archive
→ final B9 closeout
→ only then B10
```
