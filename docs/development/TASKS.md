...
- **Acceptance:** deterministic three-phase capture through read-only fakes; real B7-T02/B7-T03 pure APIs; exact passed validation and verified bundle summary; explicit call audit proving no create/upsert/delete/routing/ingest operation; random safe ephemeral prior collection; no access, create, write or delete of `local_kb_default`; failure injection remains fail-closed; no production-release claim; focused/full gates and independent review.
