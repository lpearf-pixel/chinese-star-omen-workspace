...
- **Decision:** remove automatic `pull_request` and branch `push` triggers from the eight existing Runner workflows, expose them as reusable `workflow_call` units, and compose them from one tag-filtered major-version gate. The only entry ref is a lightweight tag named `kaiyuan-runner/v2/<full-lowercase-commit-sha>`; its suffix, GitHub event SHA, Git commit object and checked-out `HEAD` must agree. Current `stable/kaiyuan-v2` must be the exact merge base and the candidate must be strictly ahead.
- **Evidence:** the finalizer records preflight plus all eight reusable workflow conclusions in `major-version-runner-result/v1` and uploads that JSON with a SHA-256 sidecar. Missing, skipped, cancelled or failed work is a failed gate.
