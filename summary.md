...
## 9. B10 Reviewer B 辅助研究记录

### C02：天文学方法评估（暂不涉及 Reviewer 标签）

2026-08-14 在 Reviewer B 逐条审核 C02 时，单独记录以下研究性判断；本节
不是 Reviewer B 的 Formal candidate / Citation eligible / Eligibility 结论，
也不能替代真人 Reviewer 的最终选择。

- C02 的“若从东北隅入 / 若从西北隅入”更适合理解为日月食路径、初亏位置
  与方位区间的描述，即“入”是食计算中的几何/方向状态变量；不宜自动等同于
  “熒惑入氐”“客星入危”一类天体进入星宿的关系词。
- 方法本身具有合理的历史天文学几何思想：根据圆面/阴影的相对位置、尺度与
  运动方向，继续求食程、初亏方位等量；因此它不是纯占辞，而是可计算的古代
  日月食算法片段。
- 文中“以蝕行减方数”“六十乘之、十一除之”等步骤表明存在明确的数值算法链。
  但从现代天文学与圆几何角度，这类比例与常数应视为古代历算体系中的近似和
  参数化算法，不能直接当作现代精确的日月食公式。
- 后续研究应区分两个问题：① C02 是否具有天文学/历算价值；② B10 formal
  candidate 的抽取目标是否包含此类“可形式化历算算法”，还是主要针对
  天体—星宿—天象关系规则。前者可以成立，不自动推出后者的 Reviewer 标签。
- 后续若继续深挖，优先追查卷104中 C02 前后的完整算法上下文，明确“蝕行”、
  “方数”、“半位”、“間量”、“日量法”等术语的定义、单位、来源历法及与
  现代食几何量的对应关系，再决定是否建立独立的 historical astronomy
  computation 数据结构。
