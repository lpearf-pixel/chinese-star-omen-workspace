...
class TransportMixin:
    ...
    def __init__(
        ...
        self.timeout = (
            timeout if timeout is not None else cfg.kb_search_timeout_seconds
        )
        self.api_key = api_key if api_key is not None else cfg.kb_search_api_key
        self.default_collection = (
            default_collection or cfg.kb_search_default_collection
        )
    ...
    def _auth_headers(self) -> dict[str, str]:
        ...
        if key.lower() in self.INVALID_API_KEY_PLACEHOLDERS:
            raise KBSearchError(
                "Invalid KB_SEARCH_API_KEY: placeholder value detected, "
    ...
    def _wire_payload(
        ...
        if isinstance(filters, dict):
            ...
            if (
                ...
                and str(legacy) != str(current)
            ):
                raise KBSearchError(
                    "conflicting book identifiers in filters: "
                    "book_id and kb_book_id"
                )
        ...
        if (
            legacy_top is not None
            and current_top is not None
            and str(legacy_top) != str(current_top)
        ):
            raise KBSearchError(
                "conflicting top-level book identifiers: "
                "book_id and kb_book_id"
            )
        if current_top is None and legacy_top is not None:
            payload["kb_book_id"] = legacy_top
        payload.pop("book_id", None)
        # This field exists only for legacy in-process smoke diagnostics. The
        # upstream v2 contract receives explicit retrieval_stage/card_types.
    ...
    def _request(
        ...
        *,
        ...
        try:
            ...
            raise KBSearchError(
                f"kb-search request failed: method={method} "
                f"url={url} error={exc}"
            ) from exc
